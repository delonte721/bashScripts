...
nameserver=`dig ns $1 @f.root-servers.net +trace +noanswer | grep $1 | grep 'IN\sNS' | awk '{print $5}' | sort | uniq | grep dnsmadeeasy | echo * | awk '{print $1}'
#nameserver2="@${nameserver}"
#dsrecord=`dig +short $nameserver2 $1 in cds`
echo $nameserver